...
> **注意（2026-08-21追記）**: この文書はQuestionBank全量を置き換える旧手順です。令和3年・問38の2件だけを修正する場合は使用しないでください。全量クリアを伴うため、`PATCH_R3_Q38_INSTRUCTIONS.md` と `src/patchR3Q38.gs` の局所パッチを使用します。本文中の旧プロジェクト・旧DBへの参照は、対象を確認せず実行してはいけません。

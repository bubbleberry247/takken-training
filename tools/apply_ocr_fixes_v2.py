...
def apply_fixes(rows):
    ...
    for idx, row in enumerate(rows):
        qid = row.get('qId', '')
        row_number = idx + 2  # CSVの行番号(ヘッダー行が1、データは2から)
        ...
        if qid == 'R1gakkaA-028':
            ...
            if row['choiceA'] != old:
                ...
                print(f"[行{row_number}] 分数欠落修正: {qid} choiceA")
        ...
        if qid == 'R2gakkaA-024':
            ...
            if row['choiceD'] != old:
                fixes_applied['分数欠落'] += 1
                print(f"[行{row_number}] 分数欠落修正: {qid} choiceD")
        ...
        if qid == 'R1gakkaA-029':
            ...
            if row['choiceB'] != old:
                ...
                print(f"[行{row_number}] 範囲開始値欠落修正: {qid} choiceB")
        ...
        if qid == 'R2gakkaA-007':
            ...
            if row['choiceD'] != old:
                fixes_applied['範囲開始値欠落'] += 1
                print(f"[行{row_number}] 範囲開始値欠落修正: {qid} choiceD")
        ...
        for col in text_columns:
            ...
            if new != old:
                ...
                print(f"[行{row_number}] 全角数字修正: {qid} {col}")
...
def main():
    ...
    print(f"カラム数: {len(fieldnames)}カラム")
    print()
